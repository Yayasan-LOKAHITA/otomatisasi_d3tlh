...
7. Klasifikasikan IKPLHN: Sangat Rendah, Rendah, Sedang, Tinggi, Sangat Tinggi (dan Tidak Dihitung bila 0).''')
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # Simpan CRS awal GRID untuk dikembalikan di akhir
        ...
        # 2. Fix & filter Kawasan, dissolve
        kwsh_fix = processing.run(
        ...
        # 6. Keep fields & REMARK (per poligon)
        keep_fields = ['ID', 'PULAU', 'PL', fld_kpgn, 'kwshutan']
        kept = processing.run(
            'native:retainfields',
            {'INPUT': inter, 'FIELDS': keep_fields, 'OUTPUT': 'TEMPORARY_OUTPUT'},
            context=context, feedback=feedback
        )['OUTPUT']
        ...
        # 10. Kembalikan ke CRS awal & write sink
        final = processing.run(
            'native:reprojectlayer',
            {'INPUT': final_tmp, 'TARGET_CRS': crs_out, 'OUTPUT': 'TEMPORARY_OUTPUT'},
            context=context, feedback=feedback
        )['OUTPUT']
