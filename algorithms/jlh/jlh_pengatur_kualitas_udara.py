...
class JLHAirQualityRegulationAlgorithm(QgsProcessingAlgorithm):
    """
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT, self.tr("JLH Pengatur Kualitas Udara")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if pulau_data is None or len(pulau_data) == 0:
            raise QgsProcessingException(
                self.tr(
                    "Layer Penutup Lahan harus memiliki kolom "
                    '"PULAU" yang terisi.'
                )
            )
        ...
        # HELPER FUNCTIONS : load_csv_table, pl_filename,
        # sanitize_suffix, has_field
        def load_csv_table(
            ...
            for delim in (";", ","):
                uri = (
                    f"file:///{csv_abs_path}?encoding=UTF-8&"
                    f"delimiter={delim}&geomType=none"
                )
        ...
        src_idx = processing.run(
            ...
            feedback=feedback.pushInfo(
                "✅ Perhitungan JLH Pengatur Kualitas Udara berhasil"
            ),
        )["OUTPUT"]
        ...
        out_src = processing.run(
            "qgis:fieldcalculator",
            {
                ...
                "FORMULA": f"""
                CASE
                    WHEN "JLH_{self.JLH}" >= 1.0
                        AND "JLH_{self.JLH}" <= 1.8 THEN 'Sangat Rendah'
                    WHEN "JLH_{self.JLH}" > 1.8
                        AND "JLH_{self.JLH}" <= 2.6 THEN 'Rendah'
                    WHEN "JLH_{self.JLH}" > 2.6
                        AND "JLH_{self.JLH}" <= 3.4 THEN 'Sedang'
                    WHEN "JLH_{self.JLH}" > 3.4
                        AND "JLH_{self.JLH}" <= 4.2 THEN 'Tinggi'
                    WHEN "JLH_{self.JLH}" > 4.2
                        AND "JLH_{self.JLH}" <= 5.0 THEN 'Sangat Tinggi'
