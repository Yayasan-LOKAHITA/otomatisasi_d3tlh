...
class UtilsGridSGSRIAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self):
        return self.tr("grid_indonesia_generator")

    def displayName(self):
        return self.tr("Sistem Grid Skala Ragam Indonesia (SGSRI)")
    ...
    def icon(self):
        return QIcon(
            os.path.join(os.path.dirname(__file__), "01 Utilities.svg")
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                self.P_GRID_SIZE,
                self.tr("Ukuran grid"),
                options=[c[0] for c in self.GRID_CHOICES],
                defaultValue=5,
            )
        )
        self.addParameter(
        ...
        self.addParameter(
            QgsProcessingParameterField(
                self.P_F_DESA,
                self.tr("Kolom Desa/Kelurahan"),
                parentLayerParameterName=self.P_ADMIN_SRC,
                optional=True,
        ...
        self.addParameter(
            QgsProcessingParameterField(
                self.P_F_KAB,
                self.tr("Kolom Kabupaten/Kota (Contoh: WADMKK)"),
                parentLayerParameterName=self.P_ADMIN_SRC,
                optional=False,
                type=QgsProcessingParameterField.Any,
            )
        ...
        self.addParameter(
            QgsProcessingParameterField(
                self.P_F_PROV,
                self.tr("Kolom Provinsi (Contoh: WADMPR)"),
                parentLayerParameterName=self.P_ADMIN_SRC,
                optional=False,
                type=QgsProcessingParameterField.Any,
            )
    ...
    def _make_hier_id_from_index(
        self, x_sw: float, y_sw: float, d_lat: float, d_lon: float
    ) -> str:
    ...
    # MCA helper
    def _mca_one_field(
        self, grid_layer, admin_layer, id_field, src_field, feedback
    ):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if admin_vl is None:
            raise QgsProcessingException(
                self.tr("Layer batas administrasi/AOI wajib diisi.")
            )
        ...
        mem_grid = QgsVectorLayer(
            f"Polygon?crs={crs_out.authid()}", "grid_tmp", "memory"
        )
        ...
        for out_name, src_field in wanted_fields:
            ...
            tmp = self._mca_one_field(
                grid_for_join, admin_vl, "id", src_field, feedback
            )
        ...
        if sink is None:
            raise QgsProcessingException(
                self.tr("Tidak dapat membuat output sink.")
            )
