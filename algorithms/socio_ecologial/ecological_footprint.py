...
class SocioEcoEcologicalFootprintAlgorithm(QgsProcessingAlgorithm):
    ...
    def _fij_regression(idx: int, year: int) -> float:
        ...
        if idx == 0:
            val = (
                3.309562804 + 0.00279158725 * t - 0.000909623865 * t * t + 0.0000219147019 * t * t * t
            ) / 1000.0
        elif idx == 1:
            val = (
                0.6757 - 0.00442074028 * t + 0.0000856109743 * t * t - 0.00000393199234 * t * t * t
            ) / 10000.0
        elif idx == 2:
            val = (
                0.000496084258064184 + 0.000000671177176 * t - 0.0000000675256016 * t * t + 0.00000000181314847 * t * t * t
            ) / 10000.0
        elif idx == 3:
            val = (
                3.05964636457972 - 0.177706769994872 * t + 0.00496665316792903 * t * t - 0.000054827359137616 * t * t * t
            ) / 10000.0
        elif idx == 4:
            val = (
                10.4795490929006 - 0.148623145 * t + 0.000728202258 * t * t + 0.0000911033598 * t * t * t
            ) / 10000.0
        elif idx == 5:
            val = (
                6.92637146880103 - 0.0230684652 * t - 0.000327768792 * t * t + 0.00000744868383 * t * t * t
            ) / 10000.0
        elif idx == 6:
            val = (
                1.6606983384315 - 0.0134741306328057 * t + 0.000174367155979212 * t * t - 0.00000267060997992912 * t * t * t
            ) / 10000.0
        elif idx == 7:
            val = (
                0.783582927451816 - 0.000928187145033807 * t + 0.00000262580886186926 * t * t - 0.0000000706920282445655 * t * t * t
            ) / 10000.0
        elif idx == 8:
            val = 0.00005
        else:
    ...
    def _read_inputs_from_excel(
        ...
        for r, cols in rows.items():
            ...
            if bnorm.startswith("total "):
                if (c is not None and c > 0) and (
                    ...
                    prodkg = (
                        float(e) if (e is not None and e > 0) else (float(d) * 1000.0)
                    )
        ...
        if verbose:
            self._log(
                f"[DEBUG] YEAR={year} POP={pop} PAPAN_m3={papan_m3cap} BUILTUP_m2={builtup_m2cap} Kb={sandang_kb}"
            )
    ...
    def _compute_pangan(
        self, mapped: Dict[int, Tuple[float, float, str]], year: int, verbose=False
    ) -> float:
        total = 0.0
        for idx in range(9):
            if idx in mapped:
            ...
            if verbose:
                self._log(
                    f"[DEBUG] idx{idx} '{label}': FIj={FIj:.8f} ({src}) mass_kg={mass_kg:.6f} LPij={LPij:.9f}"
                )
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(self.OUTPUT, self.tr("Output table"))
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        sink, dest_id = self.parameterAsSink(
            parameters, self.OUTPUT, context, out_fields, QgsWkbTypes.NoGeometry, crs
        )
    ...
    def icon(self):
        return QIcon(
            os.path.join(os.path.dirname(__file__), "04 Demographic Modelling.svg")
        )
